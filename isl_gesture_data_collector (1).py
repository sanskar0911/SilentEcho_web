"""
ISL Gesture Data Collector

This script collects hand landmark data for training Indian Sign Language (ISL) models.
It captures video from the camera, detects two hands using MediaPipe, and saves
normalized landmark coordinates to a CSV file for ISL gesture training.

Features:
- Two-hand gesture recognition for ISL
- Data collection for custom ISL gesture labels
- Model training and testing

Usage:
Run the script and press 'T' to train a new ISL label (requires both hands).
"""
...
# =========================
def create_csv():
    """Create the CSV file with header for two-hand ISL data if it doesn't exist."""
...
def normalize_two_hands(hand1, hand2):
    """
    Normalize landmarks for two hands relative to their respective wrists.

    Args:
        hand1: List of 42 coordinates for left hand (21 x, 21 y)
        hand2: List of 42 coordinates for right hand (21 x, 21 y)

    Returns:
        List of 84 normalized coordinates (42 per hand)
    """
...
def collect_data(cam, hands):
    """
    Collect ISL gesture data for a specific label (requires two hands).
...
def load_and_train_model():
    """
    Load ISL data from CSV and train a Random Forest classifier.

    Returns:
        Trained model or None if no data found
    """
    x = []
    y = []
